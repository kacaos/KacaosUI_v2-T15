...
-------------------------------------
			...
			if( T.myclass == "DRUID" ) then
				if( C.classbar.druid ~= true ) then
					...
					G.UnitFrames.Player.EclipseBar:Size(ebWidth+1, tbHeight)
					...
					TukuiWildMushroomBar:SetPoint("BOTTOM", mover, "TOP", -1, -10)
					TukuiWildMushroomBar:SetBackdrop(nil)
					TukuiWildMushroomBar:SetBackdropColor(0, 0, 0, 0)
					TukuiWildMushroomBar:SetBackdropBorderColor(0,0,0,0)
					TukuiWildMushroomBar:Size((tbWidth*3)+2, tbHeight)
						for i = 1, 3 do
							TukuiWildMushroomBar[i]:SetFrameLevel(8)
							TukuiWildMushroomBar[i]:CreateBackdrop("Default")
								if( i == 1 ) then
									TukuiWildMushroomBar[i]:Point("TOPLEFT", TukuiWildMushroomBar, "BOTTOMLEFT", 0, 20)
									TukuiWildMushroomBar[i]:Size(tbWidth, tbHeight)
								else
...
if( T.myclass == "PRIEST" ) then
				if( C.classbar.priest ~= true ) then
					...
					TukuiStatueBar:SetPoint("BOTTOM", G.UnitFrames.Player.ShadowOrbsBar, "TOP", 0, 3)
					TukuiStatueBar:CreateBackdrop("Default")
					TukuiStatueBar:Size((tbWidth*3), tbHeight)
					for i = 1, 3  do
...
if T.myclass == "MONK" then
				if( C["classbar"].monk ~= true) then
					...
					TukuiStatueBar:ClearAllPoints()
					TukuiStatueBar:SetPoint("BOTTOM", G.UnitFrames.Player.HarmonyBar, "TOP", 0, 3)
					TukuiStatueBar:CreateBackdrop("Default")
					TukuiStatueBar:Size(ebWidth, tbHeight)
					for i = 1, 5 do
